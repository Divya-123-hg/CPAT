...
export class ReviewerComponent implements OnInit {
  ...
  fetch(selected: string) {
    this.componentService.getComponentsByStatus(selected, this).subscribe((data: ComponentItem[]) => {
      this.components = data;
    });
  }

  search(searchText: string) {
    this.componentService.getComponentsByText(searchText).subscribe((data: ComponentItem[]) => {
      this.components = data;
    });
  }
