...
app.post('/api/components/:status', async (req, res) => {
    try {
        ...
        } if (status === 'rejected' || status === 'returned' || status === 'More Details Required') {
            let result4;
            try {
...
app.post('/api/components/info/info/:id/id/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('update components set status=$3, approver_feedback=$1, approved_by=$4, approved_at=NOW(), approver_status=$3 where id=$2', [comment, id, "More Details Required", name]);
    ...
    try {
        ...
        if (result3.rows[0].status === 'More Details Required') {
            ...
            const result4 = await pool.query(
                `UPDATE reviewer_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_at=null, reviewed_at=$7, reviewer_status=$9, approver_status=$10 WHERE id = $6 returning *`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null]
            );
                console.log('working',result4.rows);
            const result2 = await pool.query(
        ...
        res.json(result, result4, result2);
    }
    catch (error) {
        res.status(500).send(error.message);
    }
