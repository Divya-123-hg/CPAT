...
app.get('/api/components/:selected/:username', async (req, res) => {
    const { status, username } = req.params.selected.toLowerCase();
    if (status == 'all') {
        const result = await pool.query('select * from components where created_by =$1', [username]);
        res.json(result.rows);
    } else {
        const result = await pool.query('select * from components where lower(status)=$1 and created_by=$2', [status, username]);
        res.json(result.rows);
    }
});

app.get('/api/components/searchText/searchText/:searchText/:username', async (req, res) => {
    const { searchText, name } = req.params.searchText.toLowerCase();
    // if (searchtext == '') {
    //     const result = await pool.query('select * from components where created_by = $1',[name]);
    //     res.json(result.rows);
    // } else {
        const result = await pool.query('select * from components where lower(name) like $1', [`%${searchText}%`]);
        res.json(result.rows);
    // }
});
...
app.post('/api/components', async (req, res) => {
    ...
    try {
        ...
        const result1 = await pool.query(
            'INSERT INTO reviewer_table(id, name, description, url, status, created_by,reviewed_by) SELECT id, name, description, url, status, created_by, reviewed_by from components WHERE id = $1 returning *',
            [id]
        );
...
app.post('/api/components/:status', async (req, res) => {
    ...
    let result, result1 = null, result2, result3;
    ...
    if (status === 'reviewed' || status === 'returned') {
        ...
        if (status === 'reviewed') {
            result1 = await pool.query(
                'INSERT INTO approver_table(id, name, description,created_by,reviewed_by) SELECT id, name, description,created_by,reviewed_by FROM reviewer_table WHERE id = $1 returning *',
                [id]
            );
        }
...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            `UPDATE COMPONENTS SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_by=$7, approved_at=$8, reviewed_at=$9, reviewed_by=$10, reviewer_status=$11, approver_status=$12 WHERE id = $6`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null, null, null]
        );
        console.log(result.rows);

        const result1 = await pool.query(
            `UPDATE reviewer_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_by=$7, approved_at=$8, reviewed_at=$9, reviewed_by=$10, reviewer_status=$11, approver_status=$12 WHERE id = $6`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null, null, null]
        );
...
app.get('/api/components/a/a/a/a/a/a/a/a/a/a/a/a/a/:name', async (req, res) => {
