...
app.get('/api/components/:selected/:username', async (req, res) => {
    ...
    if (status == 'all') {
        ...
        if (username == 'reviewer_table') {
            const result = await pool.query('select * from reviewer_table  where lower(status)=$1',[]);
            res.json(result.rows);
        } else if (username == 'approver_table') {
            const result = await pool.query('select * from approver_table  where lower(status)=$1',[]);
            res.json(result.rows);
        } else {
