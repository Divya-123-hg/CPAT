...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        console.log('lalalalall status e ge',result3.rows);
        console.log(result3.rows[0]);
        // if (result3.status == 'More Details Required') {
        if (result3.rows[0].status === 'More Details Required') {

        console.log('inside if block',result3.rows);
            const result = await pool.query(
                `update components set status=$1, description=$2 where id=$3`, [result3.rows[0].status, description, id]
            );
            const result2 = await pool.query(
               `update approver_table set status=$1, description=$2 where id=$3`, [result3.rows[0].status, description, id]
            );
        } else {
            const result = await pool.query(
