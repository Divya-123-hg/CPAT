...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        console.log('inside if block',result3.rows);
            const result = await pool.query(
                `update components set status=$1, description=$2 where id=$3`, ['Information ', description, id]
            );
            const result2 = await pool.query(
               `update approver_table set status=$1, description=$2 where id=$3`, ['Information ', description, id]
            );
        } else {
            const result = await pool.query(
