...
app.post('/api/components/info/info/:id/id/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('update components set status=$3, approver_feedback=$1, approved_by=$4,approved_at=NOW(), approver_status where id=$2', [comment, id, "More Details Required"]);
