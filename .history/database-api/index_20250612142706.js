...
const result = await pool.query(`select * from reviewer_table where notification is not null`, [name]);
        console.log(result.rows);
        res.json(result.rows);
        }else if(table==='approver_table'){
const result = await pool.query(`select * from approver_table where notification is not null`, [name]);
        console.log(result.rows);
        res.json(result.rows);
        }
