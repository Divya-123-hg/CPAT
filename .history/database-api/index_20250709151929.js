...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        if (result3.rows[0].status === 'More Details Required') {
            ...
            const result1 = await pool.query(
                `UPDATE reviewer_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_at=NOW(), reviewed_at=$9, reviewer_status=$10, approver_status=$11 WHERE id = $6 returning *`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null]
            );
