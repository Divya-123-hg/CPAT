...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        const result1 = await pool.query(
            `UPDATE reviewer_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_by=$7, approved_at=$8, reviewed_at=$9, reviewer_status=$10, approver_status=$11 WHERE id = $6 returning *`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null, null]
        );

        const result2 = await pool.query(
            // 'DELETE FROM approver_table WHERE id=$1 returning *', [id]
            `UPDATE approver_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_by=$7, approved_at=$8, reviewed_at=$9, reviewer_status=$10, approver_status=$11 WHERE id = $6 returning *`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null, null]
        );
