...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        console.log('lalalalall',result3.rows);
        if (result3 === 'More Details Required') {
            const result = await pool.query(
                `update components set status=$1, description=$2 where id=$3`, [result3, description, id]
            );
            const result2 = await pool.query(
               `update approver_table set status=$1, description=$2 where id=$3`, [result3, description, id]
            );
        } else {
            const result = await pool.query(
...
app.post('/api/components/info/info/:id/id/:id', async (req, res) => {
    ...
    const result1 = await pool.query('update components set status=$3, approver_feedback=$1 where id=$2', [comment, id, "More Details Required"]);
    res.json(result.rows);
    console.log(result.rows);
});
