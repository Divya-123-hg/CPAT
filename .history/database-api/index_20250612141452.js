    ...
    if (status === 'reviewed' || status === 'returned') {
        ...
        if (status === 'reviewed') {
            result1 = await pool.query(
                'INSERT INTO approver_table(id, name, description,created_by,reviewed_by,notifu) SELECT id, name, description,created_by,reviewed_by FROM reviewer_table WHERE id = $1 returning *',
                [id]
            );
        }
