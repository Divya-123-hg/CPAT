    ...
    if (status === 'reviewed' || status === 'returned') {
        ...
        try {
            result4 = await pool.query(
                'insert into compHistory(compid,name,description,url,status,created_by,reviewed_by,approved_by,created_at,reviewed_at,approved_at,reviewer_feedback,approver_feedback,returned_by) select id,name,description,url,status,created_by,reviewed_by,approved_by,created_at,reviewed_at,approved_at,reviewer_description,approver_feedback from components where id=$1 returning *', [id]
            );
        } catch (err) {
            console.log(err);
        }
