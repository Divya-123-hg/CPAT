...
app.post('/api/components/:status', async (req, res) => {
    ...
    if (status === 'reviewed' || status === 'returned') {
        ...
        if (status === 'reviewed') {
            ...
            //     'UPDATE reviewer_table SET reviewer_description = $1 WHERE id = $2 returning *',
            //     ['component reviewed successfully', id]
            // );
...
app.delete('/api/components/id/:name', async (req, res) => {
    const id = req.params.name;
    console.log(id);
    const result1 = await pool.query('DELETE FROM users WHERE lower(username)=lower($1) ', [id]);
    if (result1.rowCount === 0) {
        return res.status(404).json({ message: 'User not found' });
    }
    res.json({ message: 'User deleted successfully', component: result.rows[0], status });
});
