...
app.get('/api/components/searchText/searchText/:searchText/:username', async (req, res) => {
    ...
    // if (searchText.trim() == '' && username == 'reviewer_table') {
    //     const result = await pool.query('select * from reviewer_table');
    //     res.json(result.rows);
    // } else if (searchText.trim() == '' && username == 'approver_table') {
    ...
    if (username == 'reviewer_table') {
        const result = await pool.query('select * from reviewer_table where lower(name) like $1 and created_by = $2', [`%${searchText}%`,username]);
        res.json(result.rows);
