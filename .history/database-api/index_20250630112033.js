...
app.post('/api/components/:status', async (req, res) => {
    ...
    let result, result1 = null, result2, result3, result4;
    ...
    if (status === 'reviewed' || status === 'returned') {
        ...
        try {
            result4 = await pool.query(
                'insert into compHistory(compid,name,description,url,status,created_by,reviewed_by,approved_by,created_at,reviewed_at,approved_at,reviewer_feedback,approver_feedback) select id,name,description,url,status,created_by,reviewed_by,approved_by,created_at,reviewed_at,approved_at,reviewer_description,approver_feedback from components where id=$1 returning *', [id]
            );
        } catch (err) {
            console.log(err);
        }
...
app.post('/api/components/id/id/id/id/id', async (req, res) => {
    ...
    try {
        ...
        const result1 = await pool.query(
            `UPDATE reviewer_table SET name = $1, description = $2, url = $3, status = $4, created_by = $5, approved_by=$7, approved_at=$8, reviewed_at=$9, reviewed_by=$10, reviewer_status=$11, approver_status=$12 WHERE id = $6 returning *`, [name, description, url, 'Pending For Review', created_by, id, null, null, null, null, null, null]
        );

        const result2 = await pool.query('DELETE FROM approver_table WHERE id=$1 returning *', [id]);
...
app.get('/api/components/:id/id', async (requestAnimationFrame, res) => {
    try {
        const result = await pool.query('select * from compHistory where compid=$1'[]);
        res.json(result.rows);
    } catch (error) {
        res.status(500).send(error.message);
    }
